...
#Divide the data
The training set is divided to a test and training the set. While the test set will be used as validation.

Variables with more than 5% errors, or static participant data, in the training set will be removed from the dataset.
...
training <- training[varErrorRate < .05]
validation <- validation[varErrorRate < .05]
...
Lets have a look the distributions of our predictors.
```{r}

# Create a function that plots the value of "classe" against the "y" value
plotM<-function(l,index){
  
  df.temp<-training[training$classe == index,l]
  hist(df.temp,xlab="Y Value",
  main=paste("Value of key variable: ",toString(l)," and outcome ",index))
